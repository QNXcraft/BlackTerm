...
19. Install Bash on BlackBerry Passport, it's too important to have bash on it, if it's not possible, write a text here. [x] FIXED - Bundled a real static GNU Bash 5.2 (ARMv7, statically linked with musl libc) binary from robxu9/bash-static. The binary is stored in `app/src/main/assets/bash-armv7` (~1.9 MB) and is extracted to the app's private `bin/` directory on first launch. It is prepended to PATH, so typing `bash` in the terminal launches real Bash 5.2, not an sh wrapper.
